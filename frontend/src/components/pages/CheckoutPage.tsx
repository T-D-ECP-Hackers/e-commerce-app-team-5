...
function CheckoutPage() {
  ...
  function getBasket(currentBasket: basket | null) {
    if (currentBasket !== null && currentBasket?.totalProducts === 0) {
      return { totalProducts: 0, basketProducts: [] };
    }

    return currentBasket as basket;
  }

  return (
    <div
      className={
        showCheckoutSummary ? "checkout-page-summary" : "checkout-page"
      }
    >
      {showCheckoutSummary ? (
        <CheckoutSummary setShowCheckoutSummary={setShowCheckoutSummary} />
      ) : (
        <>
          <div className="header-container">
            ...
            >
              <h1>
                ...
                {getTotalCostOfProducts(basket.currentBasket)}
              </h1>
              <h1>
                Checkout -{" "}
                {basket.currentBasket == null
                  ? 0
                  : basket.currentBasket.totalProducts}{" "}
                Products
              </h1>
            </div>
          </div>
